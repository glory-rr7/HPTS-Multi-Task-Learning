...
class BE_Layer(nn.Module):
    expansion = 1

    def __init__(self, in_channels, out_channels, stride):
        super().__init__()
        self.conv1 = ScaledSpectralNormConv2d(
            in_channels,
            out_channels,
            kernel_size=3,
            stride=stride,
        ...
        self.conv2 = ScaledSpectralNormConv2d(
            out_channels,
...
class BE_Block(nn.Module):
    def __init__(self, num_layers, in_channels, out_channels, stride):
        ...
        for layer_idx in range(num_layers):
            ...
            self.layers.append(BE_Layer(layer_in, out_channels, layer_stride))
        self.shortcut = self.layers[0].shortcut if len(self.layers[0].shortcut) > 0 else None
    ...
    def forward(self, x):
        ...
        if self.shortcut is not None:
            features[0] = self.shortcut(features[0])
        return sum(features)


class ResNet_UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=3, mask_classes=4, model_name="IEXResNet"):
        super().__init__()
        ...
        self.layer1 = self._make_stage(BE_Block, 64, 64, num_blocks[0], 1)
        self.layer2 = self._make_stage(BE_Block, 64, 128, num_blocks[1], 2)
        self.layer3 = self._make_stage(BE_Block, 128, 256, num_blocks[2], 2)
        self.layer4 = self._make_stage(BE_Block, 256, 512, num_blocks[3], 2)
...
if __name__ == "__main__":
    device = "mps"
    model = ResNet_UNet(3,3,4).to(device)
